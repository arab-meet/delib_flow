...
#ifndef TIAGO_DEMOS__UTILS_HPP_
#define TIAGO_DEMOS__UTILS_HPP_
  ...
  try {
    ...
    if (!locations || !locations.IsMap()) {
      RCLCPP_ERROR(node->get_logger(),
          "Target locations YAML must be a map of name -> {frame_id, pose}");
      return;
    }
    ...
    for (auto it = locations.begin(); it != locations.end(); ++it) {
      try {
        ...
        if (!entry["frame_id"] || !entry["pose"]) {
          RCLCPP_ERROR(node->get_logger(), "Entry '%s' missing 'frame_id' or 'pose', skipping",
              name.c_str());
          continue;
        }
        ...
        if (!pose_node["x"] || !pose_node["y"] || !pose_node["z"] || !pose_node["yaw_deg"]) {
          RCLCPP_ERROR(node->get_logger(), "Pose for '%s' missing x/y/z/yaw_deg, skipping",
              name.c_str());
          continue;
        }
